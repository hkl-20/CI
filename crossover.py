...
class Crossover(object):
    ...
    def crossover(self, parent1, parent2, crossoverRate):
        ...
        child1.values, child2.values = np.copy(parent1.values), np.copy(parent2.values)

        rndNum = np.random.uniform(0, 1)
            
        if (rndNum < crossoverRate):
            crossover_point1, crossover_point2 = np.random.randint(0, 8), np.random.randint(1, 9)
            while(crossover_point1 == crossover_point2):
                if swap == True:
                    crossover_point2, crossover_point1 = np.random.randint(1, 9), np.random.randint(0, 8)
    ...
    def crossover_rows(self, row1, row2): 
        swap = True
        child_row1, child_row2 = np.zeros(9), np.zeros(9)
        remaining = list(range(1, 9+1))
        cycle = 0
        while((0 in child_row1) and (0 in child_row2)):
            if(cycle % 2 == 0 and swap == True): 
